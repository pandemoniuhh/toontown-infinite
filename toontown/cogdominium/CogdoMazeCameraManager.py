...
class CogdoMazeCameraManager:
    ...
    def enable(self):
        ...
        self._parentsetPos(self.toon, 0, 0, 0)
        self._parentsetHpr(self.root, 180, self._camAngle, 0)
        self.camera.reparentTo(self.parent)
        self.camera.setPos(0, self._camDistance, 0)
        self.camera.lookAt(self.toon)
        self.defaultHeight = self._parentgetZ()
        self.update(0)
    ...
    def disable(self):
        ...
        self._parentremoveNode()
        del self.parent

    def update(self, dt):
        toonPos = self.toon.getPos()
        self._parentsetPos(self.toon.getParent(), clamp(toonPos.getX(), self.minPos[0], self.maxPos[0]), clamp(toonPos.getY(), self.minPos[1], self.maxPos[1]), 0)
